    ...
    <div :ref="id" :style="styleChartName" v-if="isEchart"></div>
      <div class="l-echart-map-map" v-else >
        <div class="l-echart-map-box" 
          ...
          <img class="l-echart-map-bg" :src="img" ref="img" draggable="false" />
          <span class="l-echart-map-location" v-for="(item,index) in location" :style="{left:setPx(item.x),top:setPx(item.y)}"></span>
        </div>
      </div> 
  </div>
</template>
...
<script>
import { getObj as getMap } from '@/api/map'
...
export default {
    ...
    computed: {
        ...
        styleImgName () {
            return {
                marginLeft: this.$getPx(this.left),
    ...
    methods: {
        ...
        getMap(this.mapData).then(res => {
            ...
            const data = JSON.parse(res.data.data.f_Data);
            console.log(data,'maptest');
